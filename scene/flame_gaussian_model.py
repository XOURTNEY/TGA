...
class FlameGaussianModel(GaussianModel):
    def __init__(self, sh_degree : int, disable_flame_static_offset=False, not_finetune_flame_params=False, n_shape=300, n_expr=100):
        super().__init__(sh_degree) # 去new gaussian_model去

        self.disable_flame_static_offset = disable_flame_static_offset # false
        self.not_finetune_flame_params = not_finetune_flame_params # false
        self.n_shape = n_shape # 300
        self.n_expr = n_expr # 100
        ...
        if self.binding is None:
            self.binding = torch.arange(len(self.flame_model.faces)).cuda() # 生成 [0, 1, 2, ..., 99] 的序列
            self.binding_counter = torch.ones(len(self.flame_model.faces), dtype=torch.int32).cuda() # [1, 1, 1, ..., 1]
    ...
    def load_meshes(self, train_meshes, test_meshes, tgt_train_meshes, tgt_test_meshes):
        if self.flame_param is None:
            ...
            pose_meshes = meshes if len(tgt_meshes) == 0 else tgt_meshes # 保存需要加载的mesh
            
            self.num_timesteps = max(pose_meshes) + 1  # required by viewers
            num_verts = self.flame_model.v_template.shape[0]

            # 静态偏移（static offset）初始化
            if not self.disable_flame_static_offset: # true
                static_offset = torch.from_numpy(meshes[0]['static_offset']) # 从第一个网格数据中读取 static_offset
                if static_offset.shape[0] != num_verts: # 如果其形状与顶点数不匹配，则通过 pad 填充使其符合顶点数。
            ...
            self.flame_param = { # 参数以时间步数 T 为索引
            ...
            self.flame_param_orig = {k: v.clone() for k, v in self.flame_param.items()} # 备份初始的 FLAME 参数
        else:
    ...
    def update_mesh_by_param_dict(self, flame_param):
        # flame_param 是针对某一帧的参数，而不是包含全时间段的参数。
        ...
        verts, verts_cano = self.flame_model( # forward
            shape[None, ...],
    ...
    def update_mesh_properties(self, verts, verts_cano):
        ...
        # position 三角形质心
        self.face_center = triangles.mean(dim=-2).squeeze(0) # T

        # orientation and scale
        self.face_orien_mat, self.face_scaling = compute_face_orientation(verts.squeeze(0), faces.squeeze(0), return_scale=True) # k R
    ...
    def training_setup(self, training_args):
        ...
        # 为FLAME中的各种可学习参数（旋转、脖子、下颌、眼睛、平移、表情等）设置 requires_grad=True，允许它们在训练过程中更新。
        # # shape
        ...
        param_pose = {'params': params, 'lr': training_args.flame_pose_lr, "name": "pose"} # 1e-5
