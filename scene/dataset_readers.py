...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png"):
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        if 'camera_angle_x' in contents: # 所有相机共享的 FOV
        ...
        for idx, frame in tqdm(enumerate(frames), total=len(frames)):
            ...
            # NeRF 'transform_matrix' is a camera-to-world transform 读取相机到世界的变换矩阵
            c2w = np.array(frame["transform_matrix"])
            # change from OpenGL/Blender camera axes (Y up, Z back) to COLMAP (Y down, Z forward)
            # NeRF 使用的是 OpenGL/Blender 的相机坐标系（Y向上，Z向后），转换为 COLMAP 的相机坐标系（Y向下，Z向前）
            c2w[:3, 1:3] *= -1  # 通过将矩阵的第1列和第2列取反来实现

            # get the world-to-camera transform and set R, T
            w2c = np.linalg.inv(c2w)
            R = np.transpose(w2c[:3,:3])  # R is stored transposed转置 due to 'glm' in CUDA code
            T = w2c[:3, 3]
            ...
            #如果 frame 中包含图像的宽高信息，直接使用这些数据；否则从图像文件中获取宽高
            if 'w' in frame and 'h' in frame:
                image = None
                width = frame['w']
                height = frame['h']
            else: # 如果图像带有透明通道，透明部分将被背景色填充。
                image = Image.open(image_path)
            ...
            # 如果当前帧包含水平视角 camera_angle_x，则使用该值，否则使用共享的 FOV。
            ...
            timestep = frame["timestep_index"] if 'timestep_index' in frame else None # 时间步
            camera_id = frame["camera_index"] if 'camera_id' in frame else None # 相机 ID
