...
__device__ float4 computeCov2D(const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, float kernel_size, const float* cov3D, const float* viewmatrix)
{
	...
	glm::mat3 cov = glm::transpose(T) * glm::transpose(Vrk) * T; // 3DGS官方的Σ‘=JWΣ(JW)^T
	...
	bool prefiltered)
{
	...
	// Compute 2D screen-space covariance matrix 即为Σ'
	...
	float3 conic = { cov.z * det_inv, -cov.y * det_inv, cov.x * det_inv }; // cov的逆

	// Compute extent in screen space (by finding eigenvalues of
	// 2D covariance matrix). Use extent to compute a bounding rectangle
	// of screen-space tiles that this Gaussian overlaps with. Quit if
	// rectangle covers 0 tiles. 
	float mid = 0.5f * (cov.x + cov.z); // 对角线元素的平均值
	float lambda1 = mid + sqrt(max(0.1f, mid * mid - det)); // 特征值
	...
	conic_opacity[idx] = { conic.x, conic.y, conic.z, opacities[idx] * cov.w }; // Σ
	...
	float* __restrict__ out_color)
{
	...
	// create the ray    point_normal/length
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			float3 ray_point = { ray.x , ray.y, 1.0 }; // rg????
			...
			double min_value = -(BB/AA) * (BB/4.) + CC; // G1D的最值
			...
			float alpha = min(0.99f, con_o.w * exp(power)); // Σ‘_inv * opacity * e^(G1D最值)
	...
	while (true){
		...
		if (inside)
		{
			...
			for (int k = 0; k < num_projected; k++){
				...
				// write colors and xyz // xxadd 1128
				for (int ch = 0; ch < CHANNELS; ch++)
