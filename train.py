...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    iter_camera_train = iter(loader_camera_train) # 数据加载器的迭代器

    # xxadd 0925 根据相机视角和点云的3D空间位置，筛选出有效的点，并为这些点分配距离值，以生成一个3D过滤器
    trainCameras = scene.train_cameras[1.0].copy()
    gaussians.compute_3D_filter(cameras=trainCameras) # .filter_3D为过滤器
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
                ...
                if custom_cam != None:
                    ...
                    if gaussians.binding != None and msg['show_mesh']:
                        ...
                        if net_image is None:
                            net_image = rgb_mesh
                        else: # 将网格的透明度 alpha_mesh 应用到渲染结果 net_image 中。
        ...
        # if iteration%1000==1:
        #     # 将 tensor 转换为 [H, W, 3] 的 NumPy 数组，并缩放至 [0, 255] 范围
        #     image_np = image.permute(1, 2, 0).detach().cpu().numpy()  # 从 [3, H, W] 转为 [H, W, 3]
        #     image_np = (image_np * 255).astype('uint8')  # 将 [0, 1] 转换为 [0, 255]
        #     from PIL import Image
        #     # 用 PIL 保存图片
        #     image_pil = Image.fromarray(image_np)
        #     image_pil.save("./test/"+str(iteration)+".png")
        ...
        # xxadd 0925 : use L1 loss for the transformed image if using decoupled appearance
        if dataset.use_decoupled_appearance:
            losses['l1']  = L1_loss_appearance(image, gt_image, gaussians, viewpoint_cam.idx)

        losses['ssim'] = (1.0 - ssim(image, gt_image)) * opt.lambda_dssim

        if gaussians.binding != None: # 主要进行一个xyz和scale的修正
            if opt.metric_xyz: # False
            ...
            if opt.lambda_scale != 0: # 1.
                if opt.metric_scale: # False
            ...
            if opt.lambda_laplacian != 0: # 0.
                losses['lap'] = gaussians.compute_laplacian_loss() * opt.lambda_laplacian

        # xxadd 0925 : 新增distortion和depth_normal loss，应对几何形状
        if dataset.gof == True and iteration > 12000: # xxchange0618 12->30000
            ...
            losses['distort'] =  distortion_loss * lambda_distortion
            losses['dn'] = depth_normal_loss * lambda_depth_normal
        ...
        with torch.no_grad():
            ...
            if iteration % 10 == 0:
                ...
                if 'distort' in losses:
                    postfix["distort"] = f"{losses['distort']:.{7}f}"
                if 'dn' in losses:
                    postfix["dn"] = f"{losses['dn']:.{7}f}"
            ...
            # Log and save
            training_report(tb_writer, iteration, losses, iter_start.elapsed_time(iter_end), testing_iterations, scene, render, (pipe, background, dataset.kernel_size))
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration % opt.opacity_reset_interval == 0 or (dataset.white_background and iteration == opt.densify_from_iter):
                    gaussians.reset_opacity() # 这里也和原版不一样了

            # xxadd 0925
            if iteration % 10000 == 0 and iteration > opt.densify_until_iter:
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in tqdm(enumerate(DataLoader(config['cameras'], shuffle=False, batch_size=None, num_workers=8)), total=len(config['cameras'])):
                    ...
                    rendering = torch.clamp(renderFunc(viewpoint, scene.gaussians, *renderArgs)["render"], 0.0, 1.0)
                    # xxadd 0925
                    image = rendering[:3, :, :]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_iterations", nargs="+", type=int, default=[5000, 7000, 10000, 12_000, 50_000, 100_000, 150_000, 200_000, 300_000, 400_000, 500_000])
