...
class FlameHead(nn.Module):
    ...
    def __init__(
        ...
        self.n_shape_params = shape_params # 300
        self.n_expr_params = expr_params # 100
        ...
        # The vertices of the template model 模板顶点
        self.register_buffer(
            "v_template", to_tensor(to_np(flame_model.v_template), dtype=self.dtype)
        )

        # The shape components and expression 形状和表情参数
        shapedirs = to_tensor(to_np(flame_model.shapedirs), dtype=self.dtype)
        shapedirs = torch.cat(
            [shapedirs[:, :, :shape_params], shapedirs[:, :, 300 : 300 + expr_params]], # 拼接
            2,
        )
        self.register_buffer("shapedirs", shapedirs)

        # The pose components 姿态参数
        ...
        # 脖子的运动链：从父节点开始追踪脖子到头部的运动链

        # add faces and uvs 用于生成面部的 UV 纹理映射，用于渲染
        verts, faces, aux = load_obj(flame_template_mesh_path, load_textures=False)

        # 纹理 UV 坐标处理标准化 计算每个面片的 UV 坐标
