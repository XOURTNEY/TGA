...
def render_set(dataset : ModelParams, name, iteration, views, gaussians, pipeline, background, render_mesh):
    ...
    #     rendering = render(view, gaussians, pipeline, background, dataset.kernel_size)["render"]
    ...
    try:
        ...
        os.system(
            f"ffmpeg -y -framerate 25 -f image2 -pattern_type glob "
            f"-i '{render_path}/*.png' "
            f"-vf \"scale=trunc(iw/2)*2:trunc(ih/2)*2\" "
            f"-pix_fmt yuv420p {iter_path}/renders.mp4"
        )
        ...
        os.system(
            ...
            f"-i '{gts_path}/*.png' "
            f"-vf \"scale=trunc(iw/2)*2:trunc(ih/2)*2\" "
            f"-pix_fmt yuv420p {iter_path}/gt.mp4"
        )
...
def render_sets(dataset : ModelParams, iteration : int, pipeline : PipelineParams, skip_train : bool, skip_val : bool, skip_test : bool, render_mesh: bool):
    with torch.no_grad():
        ...
        if dataset.target_path != "":
            ...
            #     # cams15 = cams.cameras[0:15]
            #     # render_set(dataset, "train", scene.loaded_iter, cams15, gaussians, pipeline, background, render_mesh) # scene.getTrainCameras()
            #
            if not skip_val:
